...
Envjs.connection = function(xhr, responseHandler, data){
    ...
    if(connection){
        ...
        try{
            log.debug('contentEncoding %s', contentEncoding);
            ...
            if (xhr.status == 404){
                log.error('404 - failed to open connection stream \n %s %s',
                	e.toString(), e);
            }else{
                log.error('Unknown Error %s failed to open connection stream \n %s %s',
                	xhr.status, e.toString(), e);
            }
        }finally{
			if(connection)
